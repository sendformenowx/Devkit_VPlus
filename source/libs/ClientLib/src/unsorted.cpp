    ...
    switch (type) {
        ...
        case CHAT_Notice: {
            ...
            if (strMessageCopy.find(L"1") == 0) {
                ...
                strMessageCopy.erase(0, 1);
                tmp = local_64 + strMessageCopy;
                g_pCGInterface->ShowMessage_Purble(strMessageCopy.c_str());
                g_pCGInterface->ShowLogMessage(0x4B0082, tmp.c_str());
            }
            else if (strMessageCopy.find(L"4") == 0) {
                strMessageCopy.erase(0, 1);
                tmp = local_64 + strMessageCopy;
                g_pCGInterface->ShowMessage_Orange(strMessageCopy.c_str());
                g_pCGInterface->ShowLogMessage(0xff6347, tmp.c_str());
            } else {
                tmp = local_64 + strMessageCopy;
