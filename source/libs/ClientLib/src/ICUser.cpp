...
void CICUser::Func_15(int param_1, float *param_2) {
    ...
    RenderLeftIcons(param_1, param_2);
    RenderRightIcons(param_1, param_2);
    if (param_1 != 4) {
    ...
    if (CGame::STA_FUN_004f9d00().field_c != '\0') {
        if (this->m_titleText.length() != 0) {
            dataOut[0].x = dataOut[0].x -
                           (static_cast<float>(static_cast<short>(this->fonttexture_title.GetDimensions().width)) *
                            0.5f + 2.0f);
        }
        ...
        unsigned char level = g_pCICPlayer->m_level;
        if (level <= 20) {
...
void CICUser::RenderLeftIcons(int param_1, float* param_2)
{
    //printf("Func_15 %d %p\n", param_1, param_2);
    TRIJOB_TYPE trijobType;
    std::n_wstring guildString;
    ...
    if (CGame::STA_FUN_004f9d00().field_c != '\0') {
        ...
        std::map<std::wstring, unsigned __int32>::iterator foundedInfo = g_pCICPlayer->m_LeftCharIcons.find(GetName().c_str());
        if (foundedInfo != g_pCICPlayer->m_LeftCharIcons.end())
        {
            std::map<int, void*>::iterator foundedTheIcon = g_pCICPlayer->m_LeftIconsData.find(foundedInfo->second);
            if (foundedTheIcon != g_pCICPlayer->m_LeftIconsData.end())
            {
                lastFoundedIcon = foundedTheIcon->second;

                g_RStateMgr.SetTextureForStage(0, static_cast<const IDirect3DBaseTexture9 *>(lastFoundedIcon)); // icon\\etc\\icon_rudiment.ddj
                ...
                // maybe preparation for the next(?) icon
                dataOut[0].x = dataOut[0].x - 19.0f; // upper right
                dataOut[6].x = dataOut[6].x - 19.0f; // lower left
                dataOut[2].x = dataOut[0].x + 16.0f; // upper left
                dataOut[4].x = dataOut[6].x + 16.0f; // lower right
            }
        }
...
void CICUser::RenderRightIcons(int param_1, float *param_2) {
    ...
    if (CGame::STA_FUN_004f9d00().field_c != '\0') {
        ...
        std::map<std::wstring, unsigned __int32>::iterator foundedInfo =
                g_pCICPlayer->m_RightCharIcons.find(GetName().c_str());

        if (foundedInfo != g_pCICPlayer->m_RightCharIcons.end()) {
            std::map<int, void *>::iterator foundedTheIcon = g_pCICPlayer->m_RightIconsData.find(foundedInfo->second);
            if (foundedTheIcon != g_pCICPlayer->m_RightIconsData.end()) {
                lastFoundedIconRight = foundedTheIcon->second;

                if (trijobType == TRIJOB_NOJOB) {
                    dataOut[0].x = dataOut2x_local + 5.0f;
                    dataOut[2].x = dataOut[0].x + 16.0f;
                    dataOut[4].x = dataOut[0].x + 16.0f;
                    dataOut[6].x = dataOut[0].x;
                }
...
void CICUser::RenderUpIcons(D3DVECTOR* dataOut, float* param_2) {


    void *lastFoundedIconUp;

    std::map<std::wstring, unsigned __int32>::iterator foundedInfo =
            g_pCICPlayer->m_UpCharIcons.find(GetName().c_str());

    if (foundedInfo != g_pCICPlayer->m_UpCharIcons.end()) {
        std::map<int, void *>::iterator foundedTheIcon = g_pCICPlayer->m_UpIconsData.find(foundedInfo->second);
        if (foundedTheIcon != g_pCICPlayer->m_UpIconsData.end()) {
            lastFoundedIconUp = foundedTheIcon->second;

          /*  if (g_pCICPlayer->GetJobType() != 4) {*/
                g_RStateMgr.SetTextureForStage(0, static_cast<const IDirect3DBaseTexture9 *>(lastFoundedIconUp));
                dataOut[1].y = 0.0;
                dataOut[1].z = 0.0;
