...
CIFDiscord* CIFDiscord::DCW;
CIFWnd* CIFDiscord::DCICON;

GFX_IMPLEMENT_DYNCREATE(CIFDiscord, CIFMainFrame)
GFX_BEGIN_MESSAGE_MAP(CIFDiscord, CIFMainFrame)
                    ONG_COMMAND(GDR_OK_BTN, &CIFDiscord::On_BtnClick_1)
                    ONG_COMMAND(GDR_CANCEL_BTN, &CIFDiscord::On_BtnClick_2)
GFX_END_MESSAGE_MAP()


bool CIFDiscord::OnCreate(long ln)
{
    ...
    write->SetText(L"Do you agree to redirect to the official discord channel?");
...
void CIFDiscord::OnUpdate()
{
}

void CIFDiscord::UpdateMenuSize()
{
...
void CIFDiscord::On_BtnClick_1()
{
    ShellExecute(NULL, "open", CNetProcessIn::DiscordURL.c_str(), NULL, NULL, SW_SHOWNORMAL);

}

void CIFDiscord::On_BtnClick_2() {
    this->OnCloseWnd();
